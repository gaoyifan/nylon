...
1. #### Configure Your Router/Firewall
   ...
   If both routers use `tcp_obfuscation`, add a TCP rule matching the UDP rule's
   external and internal port numbers. UDP remains available, so both rules are
   needed to expose both candidates. See
   [TCP Obfuscation](/reference/tcp-obfuscation) for host and NAT constraints.
...
With fake-TCP enabled, the published UDP and TCP endpoint ports must use the
same number. An external router may translate that number to the node's local
port for both protocols; port translation on the Nylon host is not supported.
