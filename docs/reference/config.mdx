...
# Non-negative local penalties for each transport. The matching cost is added
# during outbound endpoint selection and once to the resulting link/route
# metric. With udp_cost: 5ms, fake TCP may win when its measured outbound score
# is up to 5ms worse than UDP. Zero keeps that transport unpenalized.
tcp_cost: 0s # default: 0s; negative values are invalid
udp_cost: 0s # default: 0s; negative values are invalid
